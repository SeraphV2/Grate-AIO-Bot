...
class AdminPanelView(discord.ui.View):
    ...
    # ---------------- DASHBOARD ---------------- #
    @discord.ui.button(label="Dashboard", emoji="🏠", style=discord.ButtonStyle.secondary, custom_id="admin_dashboard")
    async def dashboard(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.set_footer(text="Grate-AIO • Premium Server Management", icon_url=guild.icon.url if guild.icon else None)
        await interaction.response.edit_message(embed=embed, view=self)

    # ---------------- MODERATION ---------------- #
    @discord.ui.button(label="Moderation", emoji="🛡️", style=discord.ButtonStyle.primary, custom_id="admin_moderation")
    async def moderation(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(title="🛡️ Moderation Commands", colour=DANGER)
    ...
    # ---------------- CHANNELS ---------------- #
    @discord.ui.button(label="Channels", emoji="📺", style=discord.ButtonStyle.primary, custom_id="admin_channels")
    async def channels(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(title="📺 Channel Control", colour=WARNING)
    ...
    async def antinuke(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(title="🚨 Anti-Nuke System", colour=DANGER)
    ...
    # ---------------- ANTI-RAID ---------------- #
    @discord.ui.button(label="Anti-Raid", emoji="🛡️", style=discord.ButtonStyle.secondary, custom_id="admin_antiraid")
    async def antiraid(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(title="🛡️ Anti-Raid Protection", colour=PRIMARY)
        embed.add_field(name="Enable", value="`.raidenable`", inline=False)
        embed.add_field(name="Disable", value="`.raiddisable`", inline=False)
        await interaction.response.edit_message(embed=embed, view=self)

    # ---------------- BACKUPS ---------------- #
    @discord.ui.button(label="Backups", emoji="💾", style=discord.ButtonStyle.primary, custom_id="admin_backups")
    async def backups(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(title="💾 Server Backups", colour=GOLD)
    ...
    # ---------------- REACTION ROLES ---------------- #
    @discord.ui.button(label="Reaction Roles", emoji="🎭", style=discord.ButtonStyle.primary, custom_id="admin_rr")
    async def reactionroles(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(title="🎭 Reaction Roles", colour=GOLD)
        embed.add_field(name="Send reaction roles to the channel it is executed in", value="`.reactionroles`", inline=False)
        await interaction.response.edit_message(embed=embed, view=self)

    # ---------------- SUPPORT / TICKETS ---------------- #
    @discord.ui.button(label="Support", emoji="🎫", style=discord.ButtonStyle.success, custom_id="admin_support")
    async def support(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(title="🎫 Support Ticket System", colour=discord.Color.green())
...
class Admin(commands.Cog):
    ...
    async def log_action(self, ctx, action: str, target: discord.Member = None, reason: str = None):
        ...
        if log_channel is None:
            return
        embed = discord.Embed(title="Admin Action Log", color=DANGER, timestamp=datetime.utcnow())
    ...
    async def auto_action(self, ctx, member: discord.Member):
        ...
        warns_count = len(self.warnings.get(guild_id, {}).get(user_id, []))
        if warns_count == 7:
            try:
    ...
    async def warn(self, ctx, member: discord.Member, *, reason: str = "No reason provided"):
        guild_id = str(ctx.guild.id)
        user_id = str(member.id)
        self.warnings.setdefault(guild_id, {}).setdefault(user_id, []).append(reason)
        self.save_warnings()
        await ctx.send(f"⚠️ {member.mention} has been warned for: `{reason}`")
        await self.log_action(ctx, "Warn", member, reason)
        try: await member.send(f"⚠️ You have been warned in **{ctx.guild.name}** for: `{reason}`")
        except: pass
    ...
    async def kick(self, ctx, member: discord.Member, *, reason: str = None):
        ...
        if not bot_member.guild_permissions.kick_members: return await ctx.send("❌ I don't have `Kick Members` permission.")
        if member in (ctx.guild.owner, ctx.author): return await ctx.send("❌ You cannot kick this user.")
        if member.top_role >= ctx.author.top_role and ctx.author != ctx.guild.owner: return await ctx.send("❌ Cannot kick someone with equal or higher role.")
        if member.top_role >= bot_member.top_role: return await ctx.send("❌ My role is too low to kick this user.")
        try: await member.kick(reason=reason)
        except discord.Forbidden: return await ctx.send("❌ I do not have permission to kick that user.")
    ...
    async def ban(self, ctx, member: discord.Member, *, reason: str = None):
        bot_member = ctx.guild.me
        if not bot_member.guild_permissions.ban_members: return await ctx.send("❌ I don't have `Ban Members` permission.")
        if member in (ctx.guild.owner, ctx.author): return await ctx.send("❌ You cannot ban this user.")
        if member.top_role >= ctx.author.top_role and ctx.author != ctx.guild.owner: return await ctx.send("❌ Cannot ban someone with equal or higher role.")
        if member.top_role >= bot_member.top_role: return await ctx.send("❌ My role is too low to ban this user.")
        try: await member.ban(reason=reason)
        except discord.Forbidden: return await ctx.send("❌ I do not have permission to ban that user.")
    ...
    async def purge(self, ctx, amount: int = None):
        if not ctx.guild.me.guild_permissions.manage_messages: return await ctx.send("❌ I need `Manage Messages` permission to purge.")
        if amount is None:
    ...
    async def slowmode(self, ctx, duration: str = None):
        ...
        if duration is None:
            ...
            return await ctx.send(f"🕑 Current slowmode: **{current} seconds**." if current else "🕑 Slowmode is **off**.")
        if duration.lower() in ["off","disable","0"]:
            await channel.edit(slowmode_delay=0)
            await ctx.send("🟢 Slowmode disabled.")
            await self.log_action(ctx, "Slowmode disabled", reason=f"Channel: {channel.name}")
            return
        try: seconds = int(duration)
        except ValueError: return await ctx.send("❌ Invalid time. Use seconds.")
        if seconds < 0 or seconds > 21600: return await ctx.send("❌ Must be 0–21600 seconds.")
    ...
    async def lock(self, ctx):
        ...
        if overwrite.send_messages is False: return await ctx.send("🔒 Channel already locked.")
        overwrite.send_messages = False
        await ctx.channel.set_permissions(ctx.guild.default_role, overwrite=overwrite)
        await ctx.send("🔒 **Channel locked.**")
        await self.log_action(ctx, "Lock channel", reason=f"Channel: {ctx.channel.name}")
    ...
    async def unlock(self, ctx):
        overwrite = ctx.channel.overwrites_for(ctx.guild.default_role)
        if overwrite.send_messages in (None, True): return await ctx.send("🔓 Channel already unlocked.")
    ...
    async def admin(self, ctx: commands.Context):
        embed = discord.Embed(
            title="🧭 Admin Control Panel",
            description="Use the buttons below to view commands",
            colour=PRIMARY,
            timestamp=datetime.utcnow()
        )
